...
public class FP03FuctionalInterface {
  ...
  public static void main(String[] args) {
    ...
    Consumer<Integer> sysoutConsumer2 =
    ...
    numbers.stream().filter(isEven).map(square).forEach(sysoutConsumer);
    ...
    // result
    BinaryOperator<Integer> sumBinaryOperator =
    ...
    int sumOfEvenNumbers = numbers.stream().reduce(0, sumBinaryOperator);
    ...
    UnaryOperator<Integer> squareUnaryOperator2 =
    ...
    numbers.stream().filter(isEven).map(squareUnaryOperator).forEach(sysoutConsumer);

    System.out.println("\nSquares of even numbers (using unary operator): ");
    numbers.stream().filter(isEven).map(squareUnaryOperator2).forEach(sysoutConsumer);
